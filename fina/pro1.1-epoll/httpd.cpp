...
void *handleRequest(void *socket) {
	...
	//文件读取失败
	if (!socket_stream) {
		cout<<"socket_stream:"<<socket_stream<<endl;
...
_disconnect:
	 ...
	 * Disconnect.
	 */
	int res = epoll_ctl(request->efd, EPOLL_CTL_DEL, request->fd, NULL);
	if (socket_stream) {
...
void start_httpd(unsigned short port, string doc_root, int nums)
{
	...
	int nready, efd, res,i, connfd,sockfd;
	...
	tep.events = EPOLLIN; tep.data.fd = serversock;
