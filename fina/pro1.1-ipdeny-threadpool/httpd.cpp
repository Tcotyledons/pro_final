...
_disconnect:
	 ...
	 */
	time_end = clock();
	sum_time+=time_end-time_begin;
	cout<<"执行总时间:"<<sum_time / CLOCKS_PER_SEC * 1000 << "ms" << endl;
	return NULL;
}
...
void start_httpd(unsigned short port, string doc_root, int nums)
{
	...
	if(USE_POOL) threadpool_destroy(thp);
